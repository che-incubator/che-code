...
FROM fedora:latest as linux-libc-builder
...
#     fi; } \
    # dnf install -y make python3.9 gcc g++ git git-core-doc bash tar gzip rsync patch \
    # wget python3-pip \
    # && dnf -y clean all
...
#     && cd nodejs \
#     && make clean \
#     && ./configure --fully-static --enable-static --download=all \
#     && make \
#     && make install
...
RUN CACHE_NODE_PATH=/checode-compilation/.build/node/v16.17.1/linux-x64 \
    && mkdir -p $CACHE_NODE_PATH \
    && echo "caching ${CACHE_NODE_PATH}" \
