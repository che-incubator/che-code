...
# gyp_main.py is not executable on UBI9 - fix permission so node-gyp
# can build native modules (affects all architectures on this image).
...
# tsgo has no native binary for ppc64le/s390x - replace it with a no-op
# so type-check steps (noEmit only) are skipped gracefully during the gulp build.
