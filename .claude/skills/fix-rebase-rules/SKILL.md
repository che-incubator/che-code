...
- **NEVER change dependency versions without asking the user.** Versions in `.rebase/add/` and `.rebase/override/` rules (e.g. `overrides`, `dependencies`, `devDependencies`) are often pinned intentionally for CVE fixes or compatibility. If a version conflict arises (e.g. npm EOVERRIDE), report it to the user and ask how to proceed — do not silently downgrade or remove the pin. When the conflict is between an override and a direct dependency, the correct fix is usually to bump the direct dependency (via `.rebase/override/`) to match the override, not the reverse.
