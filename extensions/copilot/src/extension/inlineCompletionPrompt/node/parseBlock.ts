			...
			if (errorNode !== null) {
				...
				if (this.languageId === 'python' && version >= 14) {
					...
					if (errorNode.hasError && (errorNode.text.startsWith('"') || errorNode.text.startsWith("'"))) {
						...
						if (parentType === 'function_definition' || parentType === 'class_definition' || parentType === 'module') {
							return true;
						}
					}
				}
								...
								if (version <= 13) {
									...
									for (let i = 0; i < children.length; i++) {
										...
										if (child.type === 'formal_parameters') {
											return i + 1 === children.length || (children[i + 1]?.type === '{' && i + 2 === children.length);
										}
									}
								}
							}
...
const wasmLanguageToBlockParser: { [languageId: string /* languageId in WASMLanguage*/]: BlockParser } = {
	...
	csharp: new TreeSitterBasedBlockParser(
		/* languageId */ 'csharp',
		/* nodeMatch */ {
